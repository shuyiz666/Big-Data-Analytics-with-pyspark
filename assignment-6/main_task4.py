...
label = result.map(lambda x: (x[0],x[1])).map(lambda x: ((x[0][x[0].index('id="') + 18 : x[0].index('url')]),x[1])).map(lambda x: (x[0][:x[0].index("/")],map_to_array(x[1]))).cache()
...
for num_iter in range(200):
    ...
    # update pi
    count = c.map(lambda cat : (cat, 1)).reduceByKey(add).sortByKey(ascending = True).collectAsMap()
...
# use the new alpha to take samples from dirichlet. 
    ...
    for j in range(20):
        count = x_c.filter(lambda term : term[1] == j) \
                .map(lambda term : (term[1],term[0])) \
                .reduceByKey(np.add).map(lambda x:x[1]).collect()[0]
...
for mu in log_mu:
    ...
    sort_mylist = sorted(mylist,key=(lambda x : x[1]),reverse=True)
    sort_myrdd = sc.parallelize(sort_mylist)
    tosave.append(sort_myrdd.map(lambda x:x[0]).take(50))
...
label_c = label.zip(c)
cluster_cate = label_c.map(lambda x: (x[1],x[0][0]))


cluster_catearray = cluster_cate.map(lambda x: (x[0], LabelVec(x[1])))
cluster_catearraytotal = cluster_catearray.reduceByKey(np.add)
cluster_catearraytotal_percentage = cluster_catearraytotal.map(lambda x:(x[0],x[1]/np.sum(x[1])))
cluster_catearraytotal_percentage_sort = cluster_catearraytotal_percentage.map(lambda x:(x[0],np.argsort(-x[1])[:3],np.sort(x[1])[::-1][:3]))
cluster_cate_per = cluster_catearraytotal_percentage_sort.map(lambda x:(x[0],labels[x[1]],x[2]))
cluster_cateper = cluster_cate_per.map(lambda x:(x[0],list(zip(x[1],x[2]))))

sc.parallelize(tosave, 1).saveAsTextFile(sys.argv[2])
sc.parallelize(cluster_cateper, 1).saveAsTextFile(sys.argv[3])
sc.stop()
